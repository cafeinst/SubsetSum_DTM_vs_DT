...
lemma lemma1_adversarial_lower_bound:
  assumes n_ge2: "n \<ge> 2"
      and kk_bounds: "1 \<le> kk" "kk < n"
  shows "\<exists>as s. 
           length as = n \<and> 
...
proof -
  ...
  have "steps M (enc as s kk) \<ge> card (LHS (e_k as s kk) n) + card (RHS (e_k as s kk) n)"
...
proof -
  ...
  have "real (steps M (enc as s kk)) \<ge> 2 * sqrt ((2::real) ^ n)"
    using reads_all \<open>2 * sqrt (2 ^ n) \<le> real (card (LHS (e_k as s kk) n) + 
          card (RHS (e_k as s kk) n))\<close> by linarith
...
theorem no_polytime_on_worst_case:
  assumes kk_bounds: "1 \<le> kk" 
      and kk_always_strict: "\<And>n. n \<ge> 2 \<Longrightarrow> kk < n"
...
proof
  assume poly: "\<exists>(c::real)>0. \<exists>(d::nat).
                 \<forall>n\<ge>2. \<forall>as s. length as = n \<and> distinct_subset_sums as \<longrightarrow>
                   real (steps M (enc as s kk)) \<le> c * real n ^ d"
  
  obtain c d where c_pos: "c > 0" and
    UB: "\<forall>n\<ge>2. \<forall>as s. length as = n \<and> distinct_subset_sums as \<longrightarrow>
                  real (steps M (enc as s kk)) \<le> c * real n ^ d"
    using poly by blast
  
  (* Exponentials beat polynomials *)
  obtain N where Nbig_ceil: "\<forall>n\<ge>N. of_int \<lceil>c * real n ^ d\<rceil> < 2 * sqrt ((2::real)^n)"
    using exp_beats_poly_ceiling_strict_plain[OF c_pos] by blast
  
  have Nbig: "\<forall>n\<ge>N. c * real n ^ d < 2 * sqrt ((2::real)^n)"
  proof (intro allI impI)
    fix n assume "n \<ge> N"
    then have "of_int \<lceil>c * real n ^ d\<rceil> < 2 * sqrt ((2::real)^n)"
      using Nbig_ceil by simp
    moreover have "c * real n ^ d \<le> of_int \<lceil>c * real n ^ d\<rceil>"
      by linarith
  ...
  have n_geN: "n \<ge> N" and n_ge2: "n \<ge> 2" by (simp_all add: n_def)
  
  (* kk bounds for this specific n *)
  have kk_lt_n: "kk < n"
    using kk_always_strict[OF n_ge2] .
  
  have kk_bounds_n: "1 \<le> kk" "kk < n"
  ...
  obtain as s where
    len: "length as = n" and
    dist: "distinct_subset_sums as" and
    LB: "real (steps M (enc as s kk)) \<ge> 2 * sqrt ((2::real)^n)"
  ...
  have UB_inst: "real (steps M (enc as s kk)) \<le> c * real n ^ d"
  ...
  (* Contradiction: steps \<ge> 2\<surd>(2^n) > c\<sqdot>n^d \<ge> steps *)
